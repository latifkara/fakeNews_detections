...
def getAttributess(url):
    ...
    print('Having_IP: ' ,pd.Series(having_ip)[0])
    #'Protocol': pd.Series(protocol), 'Domain': pd.Series(domain), 'Path': pd.Series(path),
    d = {
         ...
         'statistical_report': pd.Series(statistical_report), 'tiny_url': pd.Series(tiny_url),
